...
const CourseDataExtractor = {
  ...
  extractFromCard(card) {
    ...
    const reviewsEl = card.querySelector('.review-count');
   ...
   */
  extractAll() {
    const courseCards = document.querySelectorAll('#all-courses-grid .course-card');
...
const CourseCardRenderer = {
  ...
  render(courseData) {
    ...
    else if (imageSrc.startsWith('../') || imageSrc.startsWith('./') || imageSrc.startsWith('http')) {
      // Keep as is
    }
    // Handle relative paths without prefix
    else if (!imageSrc.includes('/')) {
      imageSrc = `../../assets/image/${imageSrc}`;
    }
    // If it contains a path but doesn't match above patterns, try to preserve it
    
    // Fix link path
    let courseLink = courseData.link;
    if (courseLink && !courseLink.startsWith('http') && !courseLink.startsWith('../')) {
      courseLink = courseLink; // Keep relative for All_Courses subdirectory
    }
    
    const oldPriceDisplay = courseData.oldPrice > 0 && courseData.oldPrice > courseData.price
      ? `<span class="old-price">GH₵${courseData.oldPrice.toFixed(2)}</span>`
      : '';
    
    const card = document.createElement('div');
    card.className = 'course-card';
    card.dataset.dynamicallyRendered = 'true'; // Mark as dynamically rendered
    ...
    // Store course ID if available (for API integration)
    if (courseData.id) {
    ...
    card.innerHTML = `
      ...
      <div class="course-content">
        ...
        <div class="course-meta">
          ${courseData.duration ? `<span><i class="far fa-clock"></i> ${courseData.duration}</span>` : ''}
          ${courseData.lectures ? `<span><i class="fas fa-play-circle"></i> ${courseData.lectures}</span>` : ''}
          ${courseData.students ? `<span><i class="far fa-user"></i> ${courseData.students}</span>` : ''}
        </div>
        <div class="course-footer">
          <div class="course-pricing">
            <span class="difficulty">${courseData.difficulty}</span>
            <div class="price-wrapper">
              ${oldPriceDisplay}
              <span class="current-price">GH₵${courseData.price.toFixed(2)}</span>
            </div>
          </div>
          <div class="course-rating">
            <div class="stars">
              ${courseData.starsHTML || ''}
            </div>
            <span class="review-count">(${courseData.reviews})</span>
          </div>
        </div>
...
const LoadMore = {
  ...
  async init() {
    ...
    try {
      ...
      if (this.allCoursesData.length === 0) {
        ...
        const htmlCards = document.querySelectorAll('#all-courses-grid .course-card');
    ...
    const existingDynamicCards = Array.from(this.courseGrid.querySelectorAll('.course-card[data-dynamically-rendered="true"]'));
  ...
  cleanupHTMLCards() {
    ...
    // Get ONLY original HTML cards (not dynamically rendered ones)
    const originalCards = Array.from(this.courseGrid.querySelectorAll('.course-card:not([data-dynamically-rendered="true"])'));
